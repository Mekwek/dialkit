...
const config = JSON.stringify({ accent: 'oklch(0.65 0.18 285 / 0.6)', choices: { type: 'select', options: Array.from({ length: 40 }, (_, i) => `Option ${i + 1}`) }, radius: [12, 0, 32], precise: [0.25, 0.05, 1.05, 0.1], Details: { _collapsed: true, enabled: true, caption: { type: 'text', default: 'Example' } } });
const sources = {
  react: `import React from 'react'; import {createRoot} from 'react-dom/client'; import {DialRoot, ShortcutsMenu, useDialKitController} from 'dialkit';
    function App(){const kit=useDialKitController('React',${config},{id:'fixture',defaultCollapsed:true,shortcuts:{radius:{key:'r'}}});return <><button onClick={()=>kit.setOpen(true)}>Open panel</button><button onClick={()=>kit.setOpen(false)}>Close panel</button><output>{kit.values.accent}</output><div className="dialkit-root"><ShortcutsMenu panelId="fixture"/></div><DialRoot position={new URLSearchParams(location.search).get("position") || "bottom-right"} mode={new URLSearchParams(location.search).has('inline') ? 'inline' : 'popover'} productionEnabled theme="dark"/></>};createRoot(document.getElementById('app')).render(<App/>);`,
  solid: `import {render} from 'solid-js/web'; import {DialRoot,ShortcutsMenu,createDialKitController} from 'dialkit/solid';
    function App(){const kit=createDialKitController('Solid',${config},{id:'fixture',defaultCollapsed:true,shortcuts:{radius:{key:'r'}}});return <><button onClick={()=>kit.setOpen(true)}>Open panel</button><button onClick={()=>kit.setOpen(false)}>Close panel</button><output>{kit.values().accent}</output><div class="dialkit-root"><ShortcutsMenu panelId="fixture"/></div><DialRoot position={new URLSearchParams(location.search).get("position") || "bottom-right"} mode={new URLSearchParams(location.search).has('inline') ? 'inline' : 'popover'} productionEnabled theme="dark"/></>};render(()=> <App/>,document.getElementById('app'));`,
  vue: `import {createApp,h} from 'vue'; import {DialRoot,ShortcutsMenu,useDialKitController} from 'dialkit/vue';
    createApp({setup(){const kit=useDialKitController('Vue',${config},{id:'fixture',defaultCollapsed:true,shortcuts:{radius:{key:'r'}}});return ()=>[h('button',{onClick:()=>kit.setOpen(true)},'Open panel'),h('button',{onClick:()=>kit.setOpen(false)},'Close panel'),h('output',kit.values.value.accent),h('div',{class:'dialkit-root'},[h(ShortcutsMenu,{panelId:'fixture'})]),h(DialRoot,{productionEnabled:true,theme:'dark',position:new URLSearchParams(location.search).get('position')||'bottom-right',mode:new URLSearchParams(location.search).has('inline')?'inline':'popover'})]}}).mount('#app');`,
  svelte: `<script>import {DialRoot,ShortcutsMenu,createDialKitController} from 'dialkit/svelte'; const kit=createDialKitController('Svelte',${config},{id:'fixture',defaultCollapsed:true,shortcuts:{radius:{key:'r'}}});</script><button onclick={()=>kit.setOpen(true)}>Open panel</button><button onclick={()=>kit.setOpen(false)}>Close panel</button><output>{kit.values.accent}</output><div class="dialkit-root"><ShortcutsMenu panelId="fixture"/></div><DialRoot position={new URLSearchParams(location.search).get("position") || "bottom-right"} mode={new URLSearchParams(location.search).has('inline') ? 'inline' : 'popover'} productionEnabled theme="dark"/>`,
};
